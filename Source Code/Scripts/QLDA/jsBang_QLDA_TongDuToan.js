...
function BangDuLieu_TinhOConLai(h, c) {
    if (Bang_arrMaCot[c] == "sTenDuAn") {
        var iID_MaDanhMucDuAn = Bang_arrGiaTri[h][c + 20];
        ...
        $.getJSON(url, function (item) {
            ...
            Bang_GanGiaTriThatChoO(h, c + 21, item.iID_MaMucLucNganSach);
            Bang_GanGiaTriThatChoO(h, c + 22, item.sXauNoiMa);
        });
    }
    if (Bang_arrMaCot[c] == "sTenLoaiDieuChinh") {
        var iID_MaDanhMucDuAn = Bang_arrGiaTri[h][c + 10];
        var iID_MaMucLucNganSach = Bang_arrGiaTri[h][c + 11];

        jQuery.ajaxSetup({ cache: false });
        var vR;
        var url = jsQLDA_TongDauTu;
        url += "?iID_MaDanhMucDuAn=" + iID_MaDanhMucDuAn + "&iID_MaMucLucNganSach=" + iID_MaMucLucNganSach;
        $.getJSON(url, function (item) {
...
function BangDuLieu_onKeypress_F2(h, c) {
    if (BangDuLieu_DuocSuaChiTiet && h < Bang_nH - 1) {
        ...
        var arrTruongTien = "rSoTienDauTu,rNgoaiTe_DauTu,rSoTien,rNgoaiTe_SoTien,rNgoaiTe,rTyGia".split(',');
        ...
        var arrTruongChu = "sTenDuAn,sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG,sMoTa,sTenLoaiDieuChinh,iID_MaMucLucNganSach,sXauNoiMa".split(',');
...
/* Lấy giá trị tổng đầu tư lên tổng dợ toán */
function jsLayThongTinTongDauTu(iID_MaDanhMucDuAn, iID_MaMucLucNganSach) {
    jQuery.ajaxSetup({ cache: false });
    var vR;
    var url = jsQLDA_TongDauTu;
    url += "?iID_MaDanhMucDuAn=" + iID_MaDanhMucDuAn + "&iID_MaMucLucNganSach=" + iID_MaMucLucNganSach;
    $.getJSON(url, function (item) {
        rSoTienTongDauTu = item.rSoTien;
        rNgoaiTeTongDauTu = item.rNgoaiTe;
        sTenNgoaiTeTongDauTu = item.sTenNgoaiTe;
        parent.ChungTu_ThayDoiBangDuLieu();
    });
}
