...
function BangDuLieu_TinhOConLai(h, c) {
    if (Bang_arrMaCot[c] == "sTenDuAn") {
        var iID_MaDanhMucDuAn = Bang_arrGiaTri[h][c + 32];
        ...
        $.getJSON(url, function (item) {
            ...
            Bang_GanGiaTriThatChoO(h, c + 33, item.iID_MaMucLucNganSach);
            Bang_GanGiaTriThatChoO(h, c + 34, item.sXauNoiMa);
        });
    }
    if (Bang_arrMaCot[c] == "iQuy") {
        var iID_MaDanhMucDuAn = Bang_arrGiaTri[h][c + 22];
        var iID_MaMucLucNganSach = Bang_arrGiaTri[h][c + 23];

        jQuery.ajaxSetup({ cache: false });
        var vR;
        var url = jsQLDA_TongKeHoachVon;
        url += "?iID_MaDanhMucDuAn=" + iID_MaDanhMucDuAn + "&iID_MaMucLucNganSach=" + iID_MaMucLucNganSach + "&iLoaiKeHoachVon=3";
        $.getJSON(url, function (item) {
            Bang_GanGiaTriThatChoO(h, c + 1, item.rSoTien);
            Bang_GanGiaTriThatChoO(h, c + 2, item.rNgoaiTe);
            Bang_GanGiaTriThatChoO(h, c + 3, item.sTenNgoaiTe);
        });

        var vR;
        var url = jsQLDA_TongKeHoachVon;
        url += "?iID_MaDanhMucDuAn=" + iID_MaDanhMucDuAn + "&iID_MaMucLucNganSach=" + iID_MaMucLucNganSach + "&iLoaiKeHoachVon=1";
        $.getJSON(url, function (item) {
            Bang_GanGiaTriThatChoO(h, c + 4, item.rSoTien);
            Bang_GanGiaTriThatChoO(h, c + 5, item.rNgoaiTe);
            Bang_GanGiaTriThatChoO(h, c + 6, item.sTenNgoaiTe);
        });

        var vR;
        var url = jsQLDA_TongKeHoachVon;
        url += "?iID_MaDanhMucDuAn=" + iID_MaDanhMucDuAn + "&iID_MaMucLucNganSach=" + iID_MaMucLucNganSach + "&iLoaiKeHoachVon=2";
        $.getJSON(url, function (item) {
            Bang_GanGiaTriThatChoO(h, c + 7, item.rSoTien);
            Bang_GanGiaTriThatChoO(h, c + 8, item.rNgoaiTe);
            Bang_GanGiaTriThatChoO(h, c + 9, item.sTenNgoaiTe);
        });
    }
}
...
function BangDuLieu_onKeypress_F2(h, c) {
    if (BangDuLieu_DuocSuaChiTiet && h < Bang_nH - 1) {
        ...
        var arrTruongTien = "rKeHoachVonNamTruoc,rNgoaiTe_rKeHoachVonNamTruoc,rKeHoachVonNamNay,rNgoaiTe_rKeHoachVonNamNay,rKeHoachVonUngTruoc,rNgoaiTe_rKeHoachVonUngTruoc,rSoTienBTCCap,rNgoaiTe_rSoTienBTCCap,rTyGia_rSoTienBTCCap,rSoTienDVDeNghi,rNgoaiTe_rSoTienDVDeNghi,rTyGia_rSoTienDVDeNghi,rSoTienDuToan,rNgoaiTe_rSoTienDuToan,rTyGia_rSoTienDuToan".split(',');
