...
function BangDuLieu_TinhOConLai(h, c) {
    if (Bang_arrMaCot[c] == "sTenDuAn") {
        var iID_MaDanhMucDuAn = Bang_arrGiaTri[h][c + 46];
        ...
        $.getJSON(url, function (item) {
            ...
            Bang_GanGiaTriThatChoO(h, c + 47, item.iID_MaMucLucNganSach);
            Bang_GanGiaTriThatChoO(h, c + 48, item.sXauNoiMa);
        });
    }
    else if (Bang_arrMaCot[c] == "rDonViDeNghi") {
        var GiaTri = Bang_arrGiaTri[h][c];
        if (Bang_arrGiaTri[h][c + 18] == 0) {
            Bang_GanGiaTriThatChoO(h, c + 18, GiaTri);
        }
    }
    else if (Bang_arrMaCot[c] == "rNgoaiTe_SoTien_rDonViDeNghi") {
        var GiaTri = Bang_arrGiaTri[h][c];
        if (Bang_arrGiaTri[h][c + 18] == 0) {
            Bang_GanGiaTriThatChoO(h, c + 18, GiaTri);
        }
    }
    else if (Bang_arrMaCot[c] == "rNgoaiTe_rDonViDeNghi") {
        var GiaTri = Bang_arrGiaTri[h][c];
        if (Bang_arrGiaTri[h][c + 18] == 0) {
            Bang_GanGiaTriThatChoO(h, c + 18, GiaTri);
        }
    }
    else if (Bang_arrMaCot[c] == "rTyGia_rDonViDeNghi") {
        var GiaTri = Bang_arrGiaTri[h][c];
        if (Bang_arrGiaTri[h][c + 18] == 0) {
            Bang_GanGiaTriThatChoO(h, c + 18, GiaTri);
        }
...
function BangDuLieu_onKeypress_F2(h, c) {
    if (BangDuLieu_DuocSuaChiTiet && h < Bang_nH - 1) {
        ...
        var arrTruongTien = "rDonViDeNghi,rNgoaiTe_SoTien_rDonViDeNghi,rNgoaiTe_rDonViDeNghi,rTyGia_rDonViDeNghi,rDonViThuTamUng,rNgoaiTe_SoTien_rDonViThuTamUng,rNgoaiTe_rDonViThuTamUng,rTyGia_rDonViThuTamUng,rDonViThu,rNgoaiTe_SoTien_rDonViThu,rNgoaiTe_rDonViThu,rTyGia_rDonViThu,rCucTaiChinhDeNghi,rNgoaiTe_SoTien_rCucTaiChinhDeNghi,rNgoaiTe_rCucTaiChinhDeNghi,rTyGia_rCucTaiChinhDeNghi,rCucTaiChinhThuTamUng,rNgoaiTe_SoTien_rCucTaiChinhThuTamUng,rNgoaiTe_rCucTaiChinhThuTamUng,rTyGia_rCucTaiChinhThuTamUng,rCucTaiChinhThu,rNgoaiTe_SoTien_rCucTaiChinhThu,rNgoaiTe_rCucTaiChinhThu,rTyGia_rCucTaiChinhThu".split(',');
