...
function BangDuLieu_onCellAfterEdit(h, c) {
    ...
    var tong = 0,TongTC=0,TongHV=0;
    for (var i = 2; i < Bang_nH - 1; i++) {
        if (Bang_arrLaHangCha[i] == false) {
            tong += Bang_arrGiaTri[i][c];
            TongTC += Bang_arrGiaTri[i][11];
            TongHV += Bang_arrGiaTri[i][12];
        }
    }

    var TongCongTC = Bang_arrGiaTri[0][11];
    var TongCongHV = Bang_arrGiaTri[0][12];
    ...
    if (c != 9 && c != 10 && c != 17) {
       ...
       // Bang_GanGiaTriThatChoO(1, c, ConLai);
        var sMauSac_TuChoi = Bang_sMauSac_TuChoi;
        var sMauSac_ChuaDuyet = Bang_sMauSac_ChuaDuyet;
        var sMauSac_DaDuyet = Bang_sMauSac_DongY;
        var Thieu = "", Thua = "", Du = "";
        Bang_GanGiaTriThatChoO_colName(1, "rTuChi", ConLaiTC);
        Bang_GanGiaTriThatChoO_colName(1, "rHienVat", ConLaiHV);
        if (ConLaiTC < 0 || ConLaiHV <0)
            Bang_GanGiaTriThatChoO_colName(1, "sMauSac", sMauSac_TuChoi);
        else if (ConLaiTC == 0 && ConLaiHV==0)
            Bang_GanGiaTriThatChoO_colName(1, "sMauSac", sMauSac_DaDuyet);
        else
            Bang_GanGiaTriThatChoO_colName(1, "sMauSac", Bang_sMauSac_ChuaDuyet);
