...
<body>
    <% 
        ...
        Boolean InLienTiep = Convert.ToBoolean(ViewData["InLienTiep"]);

        string HienThiNoiDung = "block";
        string HienThiTrangIn = "none";
        if (InLienTiep == true)
        {
            HienThiNoiDung = "none";
            HienThiTrangIn = "block";
        }
        else
        {

            HienThiNoiDung = "block";
            HienThiTrangIn = "none";
        }
        ...
        DataTable dtTaiKhoan = rptKeToanTongHop_SoCai_CucController.KeToan_ToIn(iID_MaPhuongAn, KhoGiay);
        ...
        dtTaiKhoan.Dispose();
        //mat giay in
        String MatIn = Convert.ToString(ViewData["MatIn"]);
        DataTable dtMatIn = rptKeToanTongHop_SoCai_CucController.KeToan_MatIn();
        SelectOptionList slMatIn = new SelectOptionList(dtMatIn, "MatIn", "TenMatIn");
        ...
        String urlReport = "";
        if (PageLoad == "1")
            urlReport = Url.Action("ViewPDF", "rptKeToanTongHop_SoCai_Cuc", new { NamLamViec = NamLamViec, ThangLamViec = ThangLamViec, iID_MaPhuongAn = iID_MaPhuongAn, iID_MaTaiKhoan = iID_MaTaiKhoan, KhoGiay = KhoGiay, InLienTiep = InLienTiep, MatIn = MatIn });
        String URL = Url.Action("SoDoLuong", "KeToanTongHop");
        String strInBia = Url.Action("Index", "rptKeToan_InBia");
        using (Html.BeginForm("EditSubmit", "rptKeToanTongHop_SoCai_Cuc", new { ParentID = ParentID }))
        {
    %>
    <div class="box_tong">
        ...
        <div id="table_form2" class="table_form2">
            <div id="rptMain" style="width: 100%; margin: 0px auto; padding: 0px 0px; overflow: visible;
                    ...
                    <li>
                        <fieldset>
                            <legend style="font-weight: bold;">
                                <%=NgonNgu.LayXau("Điều kiện lọc")%></legend>
                            <p class="liFirst">
                                <span class="pFirst" style="font-weight: bold;">
                                    <%=NgonNgu.LayXau("Tháng") %></span><%=MyHtmlHelper.DropDownList(ParentID, slThang, ThangLamViec, "ThangLamViec", "", "class=\"input1_2\" style=\"width: 120px;\" onchange=\"CheckKhoaSo(this.value)\"")%></p>
                            <p class="liFirst">
                                <span class="pFirst" style="font-weight: bold;">
                                    <%=NgonNgu.LayXau("Năm làm việc:")%></span>
                                <%=MyHtmlHelper.TextBox(ParentID, NamLamViec, "", "", "class=\"input1_2\" style=\"width: 117px; \" disabled=\"disabled\"")%>
                            </p>
                            <p class="liFirst">
                                <span class="pFirst" style="font-weight: bold;">
                                    <%=NgonNgu.LayXau("Khổ giấy:")%></span><%=MyHtmlHelper.DropDownList(ParentID, slKhoGiay, KhoGiay, "KhoGiay", "", "class=\"input1_2\" style=\"width: 120px;\" onChange=ChonKieuIn()")%></p>
                            <p class="liFirst">
                                <span class="pFirst" style="font-weight: bold;">Hỗ trợ in nhanh</span>
                                <%=MyHtmlHelper.CheckBox(ParentID,Convert.ToString( InLienTiep), "InLienTiep", "", "onclick=\"CheckDisplay(this.checked)\"")%>
                            </p>
                        </fieldset>
                    </li>
                    <li>
                        <fieldset>
                            <legend style="font-weight: bold;">
                                <%=NgonNgu.LayXau("Phương án") %></legend>
                            <p>
                                <%=MyHtmlHelper.DropDownList(ParentID, slPhuongAn, iID_MaPhuongAn, "iID_MaPhuongAn", "", "class=\"input1_2\" style=\"width: 610px;\" size='5' tab-index='-1' onChange=ChonKieuIn()")%>
                            </p>
                        </fieldset>
                    </li>
                    <li id="idTo">
                        <fieldset>
                            <legend style="font-weight: bold;">
                                <%=NgonNgu.LayXau("Chọn tờ")%></legend>
                            <p id="<%=ParentID%>_tdNoiDung" style="display: <%=HienThiNoiDung%>;">
                                <% rptKeToanTongHop_SoCai_CucController rpt1 = new rptKeToanTongHop_SoCai_CucController();%>
                                <%=rpt1.obj_data(ParentID,iID_MaPhuongAn,iID_MaTaiKhoan,KhoGiay) %>
                            </p>
                            <div style="display: <%=HienThiTrangIn%>;" id="idMatIn">
                                <%=MyHtmlHelper.DropDownList(ParentID, slMatIn, MatIn, "MatIn", "", "class=\"input1_2\" style=\"width: 250px;\" size='5'")%>
                            </div>
                        </fieldset>
                    </li>
                </ul>
                ...
                <p style="text-align: center; padding: 4px;">
                    ...
                    <input type="button" id="Button1" class="button" value="<%=NgonNgu.LayXau("In bìa")%>"
                        onclick="javascript:location.href='<%=strInBia %>'" style="display: inline-block;
                        margin-left: 5px;" />
                    <input class="button" type="button" value="<%=NgonNgu.LayXau("Hủy")%>" onclick="Huy()"
                        style="display: inline-block; margin-left: 5px;" /></p>
            </div>
        </div>
    </div>
    ...
    <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptKeToanTongHop_SoCai_Cuc", new { NamLamViec = NamLamViec, ThangLamViec = ThangLamViec, iID_MaPhuongAn = iID_MaPhuongAn, iID_MaTaiKhoan = iID_MaTaiKhoan, KhoGiay = KhoGiay, ToSo = "1", InLienTiep = InLienTiep, MatIn = MatIn }), "Xuất ra Excel")%>
    <script type="text/javascript">
        function ChonKieuIn() {
            ...
            var url = unescape('<%= Url.Action("ds_NhomDonVi?ParentID=#0&iID_MaPhuongAn=#1&iID_MaTaiKhoan=#2&KhoGiay=#3", "rptKeToanTongHop_SoCai_Cuc") %>');
        ...
        function CheckKhoaSo(value) {
            jQuery.ajaxSetup({ cache: false });
            var url = unescape('<%= Url.Action("CheckKhoaSo?iThang=#0&iNam=#1", "rptKeToanTongHop_SoCai_Cuc") %>');
    ...
    <script type="text/javascript">
        ...
        function CheckDisplay(value) {
            ...
            if (value == true || value == 'True' || value == '1') {
                document.getElementById("<%= ParentID %>_tdNoiDung").style.display = 'none';
                document.getElementById('idMatIn').style.display = '';
            } else {

                document.getElementById("<%= ParentID %>_tdNoiDung").style.display = '';
                document.getElementById('idMatIn').style.display = 'none';
            }
