...
<body>
    <%
        ...
        if (PageLoad == "1")
        {
            ...
            {
                arrView[i] = String.Format(@"/rptTCDN_BaoCaoChiTiet/viewpdf?iID_MaDoanhNghiep={0}&iLoai={1}&iQuy={2}&MaND={3}&DVT={4}&bTrongKy={5}", arrDonVi[i], iLoai, iQuy, MaND,DVT,bTrongKy);
                Chuoi += arrView[i];
                if (i < arrDonVi.Length - 1)
                    Chuoi += ",";
            }
        }
        String urlExport = Url.Action("ExportToExcel", "rptTCDN_BaoCaoChiTiet", new { });
        using (Html.BeginForm("EditSubmit", "rptTCDN_BaoCaoChiTiet", new { ParentID = ParentID, }))
        {
    %>
    <div class="box_tong">
        ...
        <script type="text/javascript">
            ...
            function Chon() {
                ...
                var url = unescape('<%= Url.Action("Ds_DonVi?ParentID=#0&iID_MaDonVi=#1", "rptTCDN_BaoCaoChiTiet") %>');
