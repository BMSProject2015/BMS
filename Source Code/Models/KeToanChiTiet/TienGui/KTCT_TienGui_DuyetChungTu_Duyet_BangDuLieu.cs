...
{
    public class KTCT_TienGui_DuyetChungTu_Duyet_BangDuLieu : BangDuLieu
    {
        /// <summary>
        /// Hàm khởi tạo
        /// </summary>
        ...
        public KTCT_TienGui_DuyetChungTu_Duyet_BangDuLieu(String iID_MaCapPhat, String iID_MaDonVi, String dTuNgay, String dDenNgay, Dictionary<String, String> arrGiaTriTimKiem, String MaND, String IPSua)
        {
            ...
            _dtChiTiet = KTCT_TienGui_ChungTuCapThuDuyetModels.Get_Grid_dtChungTu(_iID_Ma, iID_MaDonVi, dTuNgay, dDenNgay, arrGiaTriTimKiem);
            ThemHangTongCong();
        ...
        {
            String[] arrDSTruong = "bDuyet,sSoCapPhat,sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG,sMoTa,iNgay,iThang,sTenPhongBan,sTenDonVi,sTinhChatCapThu,rSoTien,rSoTienDuyet,sNoiDung".Split(',');
           // String[] arrDSTruongTieuDe = "Duyệt,Số cấp phát,LNS,L,K,M,TM,TTM,NG,TNG,Nội dung,Ngày,Tháng,Phòng ban,Đơn vị,Tính chất,Số tiền,Số tiền duyệt,Nội dung".Split(',');
            String[] arrDSTruongTieuDe = "<input type=\"checkbox\" id=\"checkAll\" onclick=\"ChonallDV(this.checked)\">,Số cấp phát,LNS,L,K,M,TM,TTM,NG,TNG,Nội dung,Ngày,Tháng,Phòng ban,Đơn vị,Tính chất,Số tiền,Số tiền duyệt,Nội dung".Split(',');
            String[] arrDSTruongDoRong = "50,80,60,30,30,30,30,30,30,30,350,50,50,150,150,200,150,150,350".Split(',');
        ...
        protected void CapNhap_arrDuLieu()
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < _arrDSMaCot.Count - 3; j++)
                {
                    ...
                    else if (_arrDSMaCot[j] == "rSoTienDuyet")
                    {
                        _arrDuLieu[i].Add(Convert.ToString(R[_arrDSMaCot[j-1]]));
                    }
                    else
                    {
                ...
                if (i == _dtChiTiet.Rows.Count - 1)
                {
                    ...
                    _arrDuLieu[i].Add(System.Web.Configuration.WebConfigurationManager.AppSettings["Background"]);//mau duoi grid
                    _arrDuLieu[i].Add(System.Web.Configuration.WebConfigurationManager.AppSettings["MauChuTinhTong"]);
