...
{
    public class KTCT_TienMat_ChungTuModels
    {
        /// <summary>
        ...
        {
            SqlCommand cmd =
                new SqlCommand("SELECT COUNT(*) FROM KTTM_ChungTu WHERE iID_MaChungTu=@iID_MaChungTu AND iTrangThai=1");
        ...
        public static String LayMaChungTu(String sSoChungTu)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT iID_MaChungTu FROM KTTM_ChungTu WHERE iTrangThai=1 AND sSoChungTu=@sSoChungTu");
        ...
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT * FROM KTTM_ChungTu WHERE iTrangThai=1 AND iID_MaChungTu=@iID_MaChungTu");
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
        ...
        {
            int vR;
            SqlCommand cmd = new SqlCommand("SELECT MAX(Convert(int,sSoChungTu)) FROM KTTM_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec");
        ...
        {
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM KTTM_ChungTu WHERE iTrangThai=1 AND sSoChungTu=@sSoChungTu AND iNamLamViec=@iNamLamViec");
            cmd.Parameters.AddWithValue("@sSoChungTu", sSoChungTu);
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM KTTM_ChungTu WHERE sSoChungTu=@sSoChungTu AND iTrangThai=1 AND iNamLamViec=@iNamLamViec");
            cmd.Parameters.AddWithValue("@sSoChungTu", sSoChungTu);
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
            ...
            {
                cmd = new SqlCommand("SELECT COUNT(*) FROM KTTM_ChungTu WHERE iID_MaChungTu=@iID_MaChungTu AND iTrangThai=1 AND iNamLamViec=@iNamLamViec");
                cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
                cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
                Int32 vR1 = Convert.ToInt32(Connection.GetValue(cmd, 0));
                cmd.Dispose();
        ...
        {
            int vR;
            SqlCommand cmd = new SqlCommand("SELECT MAX(iSoChungTu) FROM KTTM_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec");
            cmd.Parameters.AddWithValue("@iNamLamViec",iNamLamViec);
            vR = Convert.ToInt32(Connection.GetValue(cmd, 0));
            cmd.Dispose();
            return vR;
        ...
        public static String InsertRecord(String iID_MaChungTu, NameValueCollection data, String MaND, String IPSua)
        {
            ...
            if (KiemTra_sSoChungTu(sSoChungTu,iNamLamViec))
            {
               ...
               // bang.Save();
                ...
                KTCT_TienMat_ChungTuModels.InsertDuyetChungTu(MaChungTuAddNew, MessageModels.sMoiTao, MaND, IPSua);
                dtCauHinh.Dispose();
            }
        ...
        public static int Delete_ChungTu(String iID_MaChungTu, String sIPSua, String sID_MaNguoiDungSua)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    //Xóa dữ liệu trong bảng KTTM_ChungTuChiTiet
                    SqlCommand cmd;
                    cmd = new SqlCommand("UPDATE KTTM_ChungTuChiTiet SET iTrangThai=0, sIPSua=@sIPSua, sID_MaNguoiDungSua=@sID_MaNguoiDungSua WHERE iID_MaChungTu=@iID_MaChungTu");
                    cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
        ...
        public static Boolean Update_iID_MaTrangThaiDuyet(String iID_MaChungTu, int iID_MaTrangThaiDuyet, Boolean TrangThaiTrinhDuyet, String MaND, String IPSua)
        {
            ...
            String SQL = "UPDATE KTTM_ChungTuChiTiet SET iID_MaTrangThaiDuyet=@iID_MaTrangThaiDuyet WHERE iID_MaChungTu=@iID_MaChungTu";
            if (TrangThaiTrinhDuyet)
            {
                SQL = "UPDATE KTTM_ChungTuChiTiet SET iID_MaTrangThaiDuyet=@iID_MaTrangThaiDuyet, bDongY=0, sLyDo='' WHERE iID_MaChungTu=@iID_MaChungTu";
            }
            cmd = new SqlCommand();
            cmd.Parameters.AddWithValue("@iID_MaTrangThaiDuyet", iID_MaTrangThaiDuyet);
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
        ...
        public static DataTable Get_DanhSachChungTu(String MaND, String SoChungTu, String TuNgay, String DenNgay, String iID_MaTrangThaiDuyet, int Trang = 1, int SoBanGhi = 0)
        {
            ...
            String SQL = String.Format("SELECT * FROM KTTM_ChungTu WHERE iTrangThai = 1 AND {0}", DK);
            cmd.CommandText = SQL;
        ...
        public static int Get_DanhSachChungTu_Count(String MaND, String SoChungTu, String TuNgay, String DenNgay, String iID_MaTrangThaiDuyet)
        {
            ...
            String SQL = String.Format("SELECT Count(*) FROM KTTM_ChungTu WHERE iTrangThai = 1 AND {0}", DK);
            cmd.CommandText = SQL;
            vR = Convert.ToInt32(Connection.GetValue(cmd, 0));
            cmd.Dispose();
            return vR;
        ...
        public static DataTable List_DanhSachChungTu(int iThang, int iNam, Dictionary<String, String> arrGiaTriTimKiem = null)
        {
           ...
           // String SQL = String.Format("SELECT * FROM KTTM_ChungTu WHERE {0} ORDER BY iThang,iNgay,CONVERT(int, RTrim(LTrim(sSoChungTu))) ASC", DK);
            String SQL = String.Format("SELECT iID_MaChungTu,sSoChungTu,iNgay,sNoiDung,rTongSo,iID_MaTrangThaiDuyet,convert(nvarchar(30), dNgayChungTu,103) + ' ' + convert(nvarchar(30), dNgayChungTu,108) as sNgayChungTu FROM KTTM_ChungTu WHERE {0} ORDER BY iThang,iNgay,CONVERT(int, RTrim(LTrim(sSoChungTu))) ASC", DK);
            cmd.CommandText = SQL;
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
            return vR;
        ...
        public static DataTable getLichSuChungTu(String iID_MaChungTu)
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand();
            string SQL = string.Format(@"SELECT * FROM KTTM_DuyetChungTu WHERE iID_MaChungTu=@iID_MaChungTu ORDER BY dNgayTao DESC");
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
            cmd.CommandText = SQL;
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
