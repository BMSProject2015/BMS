...
<%
    ...
    if (ViewData["KhongThemDuoc"] == "1")
    {
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                parent.alert("Không thêm được chứng từ ghi sổ. Vì số chứng từ này đã có!");
            });
        </script>
        <%
    }
    ...
    {
        %>
        <script src="<%= Url.Content("~/Scripts/KeToanTongHop/jsBang_KeToanTongHop_ChungTuChiTiet.js") %>?id=<%=DateTime.Now.ToString("YYYYMMddHHmmss") %>" type="text/javascript"></script>
        ...
        <%Html.RenderPartial("~/Views/KeToanTongHop/ChungTuChiTiet/KeToanTongHop_ChungTuChiTiet_Frame_Index_DanhSach.ascx", new { ControlID = "ChungTuChiTiet", MaND = User.Identity.Name, iID_MaChungTu = iID_MaChungTu }); %>
        <%
    }
%>
</asp:Content>
