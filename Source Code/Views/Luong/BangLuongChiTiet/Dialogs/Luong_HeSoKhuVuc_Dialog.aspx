...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    else
    {
        ...
        using (Html.BeginForm("HeSoKhuVuc_Submit", "Luong_BangLuongChiTiet", new { ParentID = ParentID, iID_MaBangLuong = iID_MaBangLuong }))
        {
    %>
    <div id="nhapform">
        <div id="form2">
            ...
            <tr>
                ...
                <%--<tr id="tr_SoTien">
                                        <td class="td_form2_td1">
                                            <div>
                                               Hệ số khu vực cũ</div>
                                        </td>
                                        <td class="td_form2_td5">
                                            <div>
                                                <%=MyHtmlHelper.TextBox(ParentID, "0", "rPhuCap_KhuVuc_HeSo_Cu", "")%>
                                            </div>
                                        </td>
                                    </tr>--%>
                <tr>
                    <td class="td_form2_td1">
                        <div>
                            Hệ số khu vực mới&nbsp;<span style="color:red">*</span></div>
                    </td>
                    <td class="td_form2_td5">
                        <div>
                            <%=MyHtmlHelper.TextBox(ParentID, "0", "rPhuCap_KhuVuc_HeSo", "", "style='width:98%;' tab-index='-1'")%>
                        </div>
                    </td>
                </tr>
                <tr>
    ...
    <script type="text/javascript">
        ...
        function ChonDonVi(value) {
            ...
            var url = unescape('<%= Url.Action("get_objHeSoKhuVucCuaDonVi?iID_MaBangLuong=#0&iID_MaDonVi=#1","Luong_BangLuongChiTiet") %>');
            url = unescape(url.replace("#0", '<%=iID_MaBangLuong %>'));
            url = unescape(url.replace("#1", value));
            $.getJSON(url, function (data) {
                document.getElementById("<%= ParentID %>_rPhuCap_KhuVuc_HeSo_Cu_show").value = FormatNumber(data, 0);
            });
        }
