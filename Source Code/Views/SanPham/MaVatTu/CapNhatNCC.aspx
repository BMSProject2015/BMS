...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    String sID_MaNguoiDungSua = User.Identity.Name;
    String IPSua = Request.UserHostAddress;

    cmd = new SqlCommand("SELECT iID_MaNhaCungCap FROM DM_VatTu_NhaCungCap WHERE iID_MaVatTu = @iID_MaVatTu");
    cmd.Parameters.AddWithValue("@iID_MaVatTu", iID_MaVatTu);
    DataTable dtVTNCC = Connection.GetDataTable(cmd);
    cmd.Dispose();
    ...
    cmd.Parameters.AddWithValue("@sID_MaNguoiDung", sID_MaNguoiDungSua);
    ...
    String SQL = "SELECT * FROM DM_NhaCungCap WHERE bHoatDong = 1";
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr>
            <td style="padding-left: 20px; padding-top: 3px; color: #ec3237; text-transform:uppercase;">
                <b>Danh sách nhà cung cấp từ <%=FromRecord%>-<%=ToRecord%> trong số <%=TotalRecords%></b>
            </td>
            <td>
                <div class="msdn" style="padding-top: 5px;">
                    <%= MyHtmlHelper.PageLinks(NgonNgu.LayXau("Trang") + ":", CurrentPage, TotalPages, x => Url.Action("CapNhatNCC", new { VTNCC_page = x, iID_MaVatTu = iID_MaVatTu, sMaVatTu = sMaVatTu, sTen = sTen }))%>
                </div>
            </td>
        </tr>
    </table>
...
<table cellpadding="0" cellspacing="0" border="0" class="table_form3" >
        <tr class="tr_form3">
            ...
            <td width="25%" align="center"><b>Địa chỉ</b></td>
            <td width="10%" align="center"><b>Số điện thoại</b></td>
            <td width="25%" align="center"><b>Đánh giá chi tiết</b></td>
            <td width="5%" align="center"><b>&nbsp;</b></td>
        </tr>
    <%
    int i, j;
    String ck = "false";
    String iID_MaNhaCungCap = "";
    for (i = 0; i < dt.Rows.Count; i++)
    {
        ck = "false";
        iID_MaNhaCungCap = Convert.ToString(dt.Rows[i]["iID_MaNhaCungCap"]);
        if (i % 2 == 0)
        {%>
        <tr >
        <%}
        ...
        <%} %>
            ...
            <% for (j = 0; j < dtVTNCC.Rows.Count; j++)
                {
                    if (iID_MaNhaCungCap == Convert.ToString(dtVTNCC.Rows[j]["iID_MaNhaCungCap"]))
                        {
                            ck = "true";
                            break;
                        }
               }%>
            <td><%= MyHtmlHelper.CheckBox(ParentID, ck, iID_MaNhaCungCap, "", "onchange=\"LuuVTNCC(this.checked,'"+iID_MaNhaCungCap+"')\"")%></td>
        </tr>
    <%
    }
    dt.Dispose();
    ...
    <script type="text/javascript">
        function LuuVTNCC(ck, iID_MaNhaCungCap) {
            var url = '<%= Url.Action("Update_VTNCC?ck=#0&iID_MaVatTu=#1&iID_MaNhaCungCap=#2&sID_MaNguoiDungSua=#3&IPSua=#4", "MaVatTu") %>';
            url = url.replace("#0", ck);
            url = url.replace("#1", "<%= iID_MaVatTu %>");
            url = url.replace("#2", iID_MaNhaCungCap);
            url = url.replace("#3", "<%= sID_MaNguoiDungSua %>");
            url = url.replace("#4", "<%= IPSua %>");
