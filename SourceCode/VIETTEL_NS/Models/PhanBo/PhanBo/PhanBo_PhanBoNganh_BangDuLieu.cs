...
{
    public class PhanBo_PhanBoNganh_BangDuLieu : BangDuLieu
    {
        ...
        /// Hàm khởi tạo
        /// </summary>
        /// <param name="iID_MaChiTieu"> Mã chỉ tiêu</param>
        /// <param name="MaND">Mã người dùng</param>
        public PhanBo_PhanBoNganh_BangDuLieu(String iID_MaChiTieu, Dictionary<String, String> arrGiaTriTimKiem, String MaND)
        {
            ...
            _dtChiTiet = PhanBo_PhanBoChiTietModels.Get_dtPhanBoNganh_ChiTiet(_iID_Ma, arrGiaTriTimKiem);
            DataRow[] arrR;
            DataRow r;
            DataTable dtTong = PhanBo_PhanBoChiTietModels.Get_dtTongPhanBoTheoMucLucNganSach(iID_MaChiTieu);
            DataTable dtChoPhanBo = PhanBo_PhanBoChiTietModels.Get_dtDonViChoPhanBo(iID_MaChiTieu);
            int i = 0;
            int h = 0, h1 = 0;
            String sXauNoiMa, sXauNoiMa1, sColName = "";
            Double rGT = 0;
            for (i = 0; i < dtTong.Rows.Count; i++)
            {
                sXauNoiMa = Convert.ToString(dtTong.Rows[i]["sXauNoiMa"]);
                rGT = 0;
                for (h = h1; h < _dtChiTiet.Rows.Count; h++)
                {
                    sXauNoiMa1 = Convert.ToString(_dtChiTiet.Rows[h]["sXauNoiMa"]);
                    if (sXauNoiMa.Equals(sXauNoiMa1))
                    {
                        r = _dtChiTiet.NewRow();
                        for (int j = 0; j < _dtChiTiet.Columns.Count; j++)
                        {
                            sColName = _dtChiTiet.Columns[j].ColumnName;
                            for (int c = 0; c < dtChoPhanBo.Rows.Count; c++)
                            {
                                sXauNoiMa1 = Convert.ToString(dtChoPhanBo.Rows[c]["sXauNoiMa"]);
                                if (sXauNoiMa.Equals(sXauNoiMa1) && sColName.StartsWith("r") && sColName.IndexOf("PhanBoNganh") < 0)
                                {
                                    rGT = Convert.ToDouble(dtChoPhanBo.Rows[c][sColName]);
                                    break;
                                }

                            }
                            if (sColName.StartsWith("r") && sColName.IndexOf("PhanBoNganh") < 0)
                            {
                                r[sColName] = Convert.ToDouble(dtTong.Rows[i][sColName]) + rGT;
                            }
                            else if (sColName.IndexOf("PhanBoNganh") < 0)
                            {
                                r[sColName] = _dtChiTiet.Rows[h][sColName];
                            }
                        }
                        r["bLaHangCha"] = true;
                        r["iID_MaMucLucNganSach"] = Guid.Empty;
                        r["sXauNoiMa"] = "";    
                        _dtChiTiet.Rows.InsertAt(r, h);
                        h1 = h + 1;
                        break;
                    }
                }
        ...
        protected void CapNhapDanhSachMaHang()
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                String MaHang = String.Format("{0}_{1}_{2}", R["iID_MaMucLucNganSach"], R["iID_MaDonVi"], R["iID_MaPhanBoChiTiet"]);
        ...
        protected void CapNhapDanhSachMaCot_Slide()
        {
            ...
            String[] arrDSTruongTien = "rNgay_ChiTieu,rNgay,rSoNguoi_ChiTieu,rSoNguoi,rChiTaiKhoBac_ChiTieu,rChiTaiKhoBac,rTonKho_ChiTieu,rTonKho,rTuChi_ChiTieu,rTuChi,rChiTapTrung_ChiTieu,rChiTapTrung,rHangNhap_ChiTieu,rHangNhap,rHangMua_ChiTieu,rHangMua,rHienVat_ChiTieu,rHienVat,rDuPhong_ChiTieu,rDuPhong,rPhanCap_ChiTieu,rPhanCap".Split(',');//MucLucNganSachModels.strDSTruongTien.Split(',');
            String[] arrDSTruongTienTieuDe = "Ngày DV,Ngày,Người DV,Người,Chi tại kho bạc DV,Chi tại kho bạc,Tồn kho DV,Tồn kho,Đơn vị đề nghị TC,Tự chi,Chi tập trung DV,Chi tập trung,Hàng nhập DV,Hàng nhập,Hàng mua DV,Hàng mua,Đon vị đề nghị HV,Hiện vật,Dự phòng DV,Dự phòng,Phân cấp DV,Phân cấp".Split(',');//MucLucNganSachModels.strDSTruongTienTieuDe.Split(',');
            String[] arrDSTruongTienDoRong = "100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100".Split(',');//MucLucNganSachModels.strDSTruongTienDoRong.Split(',');
        ...
        protected void CapNhapHangTongCong()
        {
                ...
                if (_arrLaHangCha[i])
                {
                    for (int k = _nCotFixed + 1; k < _arrDSMaCot.Count - 2; k++)
                    {
                        if (_arrDSMaCot[k].StartsWith("r"))
                        {
                            ...
                            if (_dtChiTiet.Rows[i]["iID_MaMucLucNganSach"].Equals(Guid.Empty) == false)
                            {
            ...
            for (int i = _dtChiTiet.Rows.Count - 1; i >= 0; i--)
            {
                double S = 0;
                int k;
                for (k = _nCotFixed + 1; k < _arrDSMaCot.Count - 2; k++)
                {
