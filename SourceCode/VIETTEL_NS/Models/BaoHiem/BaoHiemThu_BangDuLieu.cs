...
namespace VIETTEL.Models
{
    ...
    /// </summary>
    public class BaoHiemThu_BangDuLieu:BangDuLieu
    {
        /// <summary>
        /// Hàm khởi tạo
        /// </summary>
        ...
        public BaoHiemThu_BangDuLieu(String iID_MaChungTuThu, Dictionary<String, String> arrGiaTriTimKiem, String MaND, String IPSua)
        {
            this._iID_Ma = iID_MaChungTuThu;
            this._MaND = MaND;
            this._IPSua = IPSua;

            String SQL;
            SqlCommand cmd;
            SQL = "SELECT * FROM BH_ChungTuThu WHERE iID_MaChungTuThu=@iID_MaChungTuThu AND iTrangThai=1";
            cmd = new SqlCommand();
            cmd.Parameters.AddWithValue("@iID_MaChungTuThu", _iID_Ma);
            cmd.CommandText = SQL;
            ...
            Boolean ND_DuocSuaChungTu = LuongCongViecModel.NguoiDung_DuocSuaChungTu(BaoHiemModels.iID_MaPhanHe, MaND, iID_MaTrangThaiDuyet);
            if (LuongCongViecModel.KiemTra_TrangThaiDaDuyet(BaoHiemModels.iID_MaPhanHe, iID_MaTrangThaiDuyet))
            {
                _ChiDoc = true;
            }
            ...
            if (LuongCongViecModel.KiemTra_TrangThaiTrinhDuyet(BaoHiemModels.iID_MaPhanHe, iID_MaTrangThaiDuyet) &&
                ND_DuocSuaChungTu)
            {
                _CoCotDuyet = true;
                _DuocSuaDuyet = true;
            ...
            if (LuongCongViecModel.KiemTra_TrangThaiTuChoi(BaoHiemModels.iID_MaPhanHe, iID_MaTrangThaiDuyet))
            {
                _CoCotDuyet = true;
            }

            _DuocSuaChiTiet = LuongCongViecModel.NguoiDung_DuocThemChungTu(BaoHiemModels.iID_MaPhanHe, MaND);

            _dtChiTiet = BaoHiem_ChungTuThuChiTietModels.Get_dtBaoHiemThuChiTiet(_iID_Ma, arrGiaTriTimKiem);
        ...
        protected void CapNhapDanhSachMaHang()
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                String MaHang = Convert.ToString(R["iID_MaChungTuThuChiTiet"]);
        ...
        protected void CapNhapDanhSachMaCot_Fixed()
        {
            ...
            String[] arrDSTruong = BaoHiemModels.strDSTruong.Split(',');
            String[] arrDSTruongTieuDe = BaoHiemModels.strDSTruongTieuDe.Split(',');
            String[] arrDSTruongTien = BaoHiemModels.strDSTruongTien.Split(',');
            String[] arrDSTruongTienTieuDe = BaoHiemModels.strDSTruongTienTieuDe.Split(',');
            String[] arrDSTruongTienDoRong = BaoHiemModels.strDSTruongTienDoRong.Split(',');
            String[] arrDSTruongDoRong = BaoHiemModels.strDSTruongDoRong.Split(',');
        ...
        {
            String[] arrDSTruongTien = (BaoHiemModels.strDSTruongTien + ",sTenLoaiBaoHiem").Split(',');//MucLucNganSachModels.strDSTruongTien.Split(',');
            String[] arrDSTruongTienTieuDe = (BaoHiemModels.strDSTruongTienTieuDe + ",Loại bảo hiểm" ).Split(','); //MucLucNganSachModels.strDSTruongTienTieuDe.Split(',');
            String[] arrDSTruongTienDoRong = (BaoHiemModels.strDSTruongTienDoRong + ",100").Split(',');
        ...
        protected void CapNhap_arrEdit()
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < _arrDSMaCot.Count; j++)
                {
                    ...
                    {
                        //Cot tien
                        if (_arrDSMaCot[j] != "sTenLoaiBaoHiem" && _DuocSuaChiTiet &&
                                _ChiDoc == false &&
                                okHangChiDoc == false)
                        {
                            okOChiDoc = false;
