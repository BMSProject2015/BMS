...
<%
    ...
    {
        cmd.CommandText = "SELECT * FROM KT_TaiKhoan WHERE iID_MaTaiKhoan_Cha=@iID_MaTaiKhoan_Cha AND iTrangThai = 1 AND iNam = @NamLamViec ORDER BY iSTT";
        cmd.Parameters.AddWithValue("@iID_MaTaiKhoan_Cha", iID_MaTaiKhoan_Cha);
    }
    else
    {
        cmd.CommandText = "SELECT * FROM KT_TaiKhoan WHERE (iID_MaTaiKhoan_Cha is Null OR iID_MaTaiKhoan_Cha='') AND iTrangThai = 1 AND iNam = @NamLamViec ORDER BY iSTT";
    }
    cmd.Parameters.AddWithValue("@NamLamViec", DanhMucModels.NamLamViec(User.Identity.Name));
    DataTable dt = Connection.GetDataTable(cmd);

    using (Html.BeginForm("SortSubmit", "LoaiTaiKhoan", new { iID_MaTaiKhoan_Cha = iID_MaTaiKhoan_Cha }))
    {   
%>
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (i == 0)
            {
                strSelected = "selected";
                                ...
                                <option value="<%= Row["iID_MaTaiKhoan"]%>" "<%= strSelected%>"><%= Row["iID_MaTaiKhoan"]%> - <%= Row["sTen"]%></option>
                            <%}
        dt.Dispose();    
                            %>                                 
                    </select>
