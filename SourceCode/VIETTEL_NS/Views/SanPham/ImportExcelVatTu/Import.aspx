...
<%  
    ...
    if (iID_MaDonViDangNhap == "-1")
        dataRow = dtSheet.Select("TABLE_NAME IN ('NhomLoaiVatTu$','NhomChinh$','NhomPhu$','ChiTietVatTu$','TinhTrang$','DonViTinh$','VatTu$')");
    else
    {
        if (TImport == "VatTu")
            dataRow = dtSheet.Select("TABLE_NAME IN ('VatTu$')");    
        else if (TImport == "TonKho")
            dataRow = dtSheet.Select("TABLE_NAME IN ('TonKho$')");    
    }
    ...
    connExcel.Close();
    using (Html.BeginForm("EditSubmit", "ImportExcelVatTu", new { ParentID = ParentID }))
    {          
%>
...
<script type="text/javascript">
    function btnXem() {
        ...
        var url = unescape('<%= Url.Action("get_dtSheet?ParentID=#0&FilePath=#1&SheetName=#2", "ImportExcelVatTu") %>');
