        ...
        <tr>
            ...
            <td style="width: 50%">
                <div class="msdn" style="padding-top: 5px;">
                    <%if (Searchid == "1"){ %>
                        <div class="msdn" style="padding-top: 5px;">
                        <%= MyHtmlHelper.PageLinksAjax(NgonNgu.LayXau("Trang") + ":", CurrentPage, TotalPages, (x, y) => Ajax.ActionLink(y, "Index", "Ajax", new { PartialView = "~/Views/DuyetVatTu/Detail.aspx", OnLoad = ParentID + "_OnLoad", TimKiemVatTu_page = x, sMaVatTu = sMaVatTu, sTen = sTen, sTenGoc = sTenGoc, sQuyCach = sQuyCach, cbsMaVatTu = cbsMaVatTu, cbsTen = cbsTen, cbsTenGoc = cbsTenGoc, cbsQuyCach = cbsQuyCach, Searchid = Searchid }, new AjaxOptions { }).ToString())%>
                        </div>
                    <%}else{ %>
                        <div class="msdn" style="padding-top: 5px;">
                        <%= MyHtmlHelper.PageLinksAjax(NgonNgu.LayXau("Trang") + ":", CurrentPage, TotalPages, (x, y) => Ajax.ActionLink(y, "Index", "Ajax", new { PartialView = "~/Views/DuyetVatTu/Detail.aspx", OnLoad = ParentID + "_OnLoad", TimKiemVatTu_page = x, MaNhomLoaiVatTu = iDM_MaNhomLoaiVatTu, MaNhomChinh = iDM_MaNhomChinh, MaNhomPhu = iDM_MaNhomPhu, MaChiTietVatTu = iDM_MaChiTietVatTu, MaXuatXu = iDM_MaXuatXu, iTrangThai = iTrangThai, Searchid = Searchid }, new AjaxOptions { }).ToString())%>
                        </div>
                    <%} %>
                </div>
            </td>
...
<table cellpadding="0" cellspacing="0" border="0" class="table_form3" >
    ...
    { 
        ...
        for (i = 0; i < dt.Rows.Count; i++)
        {
            ...
            String sMaVatTuClick = Convert.ToString(dt.Rows[i]["sMaVatTu"]);
            String sMaVatTuYCClick = Convert.ToString(dt.Rows[i]["sMaYeuCau"]);
        ...
        <%} %>
            <td><%=i+1 %></td>
            <td onclick="ChonMaYCDeLayLichSu(<%=sMaVatTuYCClick %>);"><b><%= dt.Rows[i]["sMaYeuCau"]%></b></td>
            <td onclick="ChonMaDeLayLichSu(<%=sMaVatTuClick %>);"><b><%= dt.Rows[i]["sMaVatTu"]%></b></td>
            <td><%=dt.Rows[i]["sTen"]%></td>
