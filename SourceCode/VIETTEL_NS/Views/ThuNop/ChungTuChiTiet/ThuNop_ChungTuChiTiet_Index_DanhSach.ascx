...
<%
    ...
    ThuNop_ChungTu_BangDuLieu bang = new ThuNop_ChungTu_BangDuLieu(iID_MaChungTu, arrGiaTriTimKiem, MaND, IPSua);
...
<form action="<%=Url.Action("SearchSubmit","ThuNop_ChungTuChiTiet",new {ParentID = ParentID, iID_MaChungTu = iID_MaChungTu})%>" method="post">
    <table class="mGrid1">
    <tr>
      <%--  <%
...
<%  
    ...
    <form action="<%=Url.Action("DetailSubmit", "ThuNop_ChungTuChiTiet", new{iID_MaChungTu=iID_MaChungTu})%>" method="post">
    <%
    } %>
