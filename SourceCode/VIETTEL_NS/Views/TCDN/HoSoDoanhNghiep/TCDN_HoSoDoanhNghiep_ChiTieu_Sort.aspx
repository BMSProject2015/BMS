...
<%
    ...
    {
        cmd.CommandText = "SELECT * FROM TCDN_HoSoDoanhNghiep_ChiTieu WHERE iID_MaChiTieuHoSo_Cha=@iID_MaChiTieuHoSo_Cha AND iTrangThai = 1 ORDER BY iSTT";
        cmd.Parameters.AddWithValue("@iID_MaChiTieuHoSo_Cha", iID_MaChiTieuHoSo_Cha);
    }
    else
    {
        cmd.CommandText = "SELECT * FROM TCDN_HoSoDoanhNghiep_ChiTieu WHERE iID_MaChiTieuHoSo_Cha = 0 AND iTrangThai = 1 ORDER BY iSTT";
    }
    DataTable dt = Connection.GetDataTable(cmd);

    using (Html.BeginForm("SortSubmit", "TCDN_HoSoDoanhNghiep_ChiTieu", new { iID_MaChiTieuHoSo_Cha = iID_MaChiTieuHoSo_Cha }))
    {   
%>
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (i == 0)
            {
                strSelected = "selected";
                                ...
                                <option value="<%= Row["iID_MaChiTieuHoSo"]%>" "<%= strSelected%>"><%= Row["sKyHieu"]%> - <%= Row["sTen"]%></option>
                            <%}
        dt.Dispose();    
                            %>                                 
                    </select>
