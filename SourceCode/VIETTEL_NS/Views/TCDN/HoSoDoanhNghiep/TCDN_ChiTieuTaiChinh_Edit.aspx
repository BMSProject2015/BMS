...
<script src="<%= Url.Content("~/Scripts/TCDN/jsTCDN_ChiTieuTaiChinh.js") %>?id=<%=DateTime.Now.ToString("YYYYMMddHHmmss") %>" type="text/javascript"></script>
...
<%
    ...
    //Update số liệu của bảng cân đối tài khoản và bảng kinh doanh
    //TCDN_ChiTieuTaiChinhModels.LaySoLieuCanDoiTaiKhoanVaKinhDoanh(iID_MaDoanhNghiep, iNam, iQuy);
...
<div class="box_tong">
    <div class="title_tong">
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
        	<tr>
            	<td>
                    <span>Chỉ tiêu tài chính quý <%=iQuy %> năm <%=iNam%> của doanh nghiệp <%=sTenDoanhNghiep %></span>
                </td>
            </tr>
        </table>
    </div>
    <div id="nhapform">
        <div id="form2" style="padding: 10px;">
                        ...
                        <div>
                            <script type="text/javascript">
                                function ddlDoanhnghiep_SelectedValueChanged(ctl) {
                                    var url = "<%=Url.Action("List", "TCDN_ChiTieuTaiChinh", new { iQuy=iQuy,iNam=iNam})%>";
                        ...
                        <div>
                            <script type="text/javascript">
                                function ddlQuy_SelectedValueChanged(ctl) {
                                    var url = "<%=Url.Action("List", "TCDN_ChiTieuTaiChinh", new {iNam=iNam,iID_MaDoanhNghiep=iID_MaDoanhNghiep})%>";
                                    if(ctl.selectedIndex>=0)
                                    {
                                        var value = ctl.options[ctl.selectedIndex].value;
                                        if(value!="")
            ...
            <br />
            <%Html.RenderPartial("~/Views/TCDN/HoSoDoanhNghiep/TCDN_ChiTieuTaiChinh_DanhSach.ascx", new { ControlID = "ChungTuChiTiet", MaND = User.Identity.Name }); %>    
        </div>
    </div>
</div>
</asp:Content>
