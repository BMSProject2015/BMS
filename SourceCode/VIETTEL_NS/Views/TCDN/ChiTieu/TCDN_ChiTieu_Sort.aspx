...
<%
    ...
    {
        cmd.CommandText = "SELECT * FROM TCDN_ChiTieu WHERE iID_MaChiTieu_Cha=@iID_MaChiTieu_Cha AND iTrangThai = 1 ORDER BY iSTT";
        cmd.Parameters.AddWithValue("@iID_MaChiTieu_Cha", iID_MaChiTieu_Cha);
    }
    else
    {
        cmd.CommandText = "SELECT * FROM TCDN_ChiTieu WHERE iID_MaChiTieu_Cha = 0 AND iTrangThai = 1 ORDER BY iSTT";
    }
    DataTable dt = Connection.GetDataTable(cmd);

    using (Html.BeginForm("SortSubmit", "TCDN_ChiTieu", new { iID_MaChiTieu_Cha = iID_MaChiTieu_Cha }))
    {   
%>
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (i == 0)
            {
                strSelected = "selected";
                                ...
                                <option value="<%= Row["iID_MaChiTieu"]%>" "<%= strSelected%>"><%= Row["sKyHieu"]%> - <%= Row["sTen"]%></option>
                            <%}
        dt.Dispose();    
                            %>                                 
                    </select>
