    ...
    for (c = 0; c < Bang_nC; c++) {
        if (Bang_arrMaCot[c].startsWith('r')) {
            ...
            for (h = 0; h < Bang_nH - 1; h++) {
                var bLoai = Bang_arrGiaTri[h][1];
                if (bLoai == '0') {
                    SC += Bang_arrGiaTri[h][c];
                }
                else {
                    ST += Bang_arrGiaTri[h][c];
                }
            }
            S += SC - ST;
...
function BangDuLieu_onKeypress_F2(h, c) {
    if (BangDuLieu_DuocSuaChiTiet && h < Bang_nH - 1) {
        ...
        var arrTruongChu = "bLoai,sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG,sMoTa,iID_MaMucLucNganSach,sXauNoiMa,iNgay,iThang,iID_MaPhongBan,sTenPhongBan,iID_MaDonVi,sTenDonVi,iID_MaTinhChatCapThu,sTinhChatCapThu,sNoiDung".split(',');
