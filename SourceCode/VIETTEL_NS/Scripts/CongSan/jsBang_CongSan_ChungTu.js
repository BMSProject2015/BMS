    ...
    if (Bang_ChiDoc == false) {
        ...
        for (j = 0; j < Bang_nC; j++) {
            if (Bang_arrMaCot[j] == "sSoChungTu" ||
                ...
                Bang_arrEdit[0][j] = true;
            }
            else {
                Bang_arrEdit[0][j] = false;
            }
        }

        BangDuLieu_CapNhapSTT();

        var url = urlServerPath + "CongSan_ChungTuChiTiet/get_ThongTinChungTuMoi";
        $.getJSON(url, function (item) {
            var cs;
            var iID_MaChungTu = item.iID_MaChungTu;
            cs = Bang_arrCSMaCot["sSoChungTu"];
            Bang_GanGiaTriThatChoO(0, cs, item.sSoChungTu);
            cs = Bang_arrCSMaCot["iNgay"];
            Bang_GanGiaTriThatChoO(0, cs, item.iNgay);
            //Bang_HideCloseDialog();
            //Sua MaHang="": Day la hang them moi
            Bang_arrMaHang[0] = iID_MaChungTu;
            cs = Bang_arrCSMaCot["sSoChungTu"];
