...
function BangDuLieu_TinhOConLai(h, c) {
    if (Bang_arrMaCot[c] == "sSoChungTuCapThu") {
        var iID_MaChungTu_Duyet = Bang_arrGiaTri[h][c + 29];
        jQuery.ajaxSetup({ cache: false });
        var vR;
        var url = jsKTCT_CapThu_Duyet;
        url += "?iID_MaChungTu_Duyet=" + iID_MaChungTu_Duyet;
        $.getJSON(url, function (item) {
            Bang_GanGiaTriThatChoO(h, c + 16, item.sNoiDung);
            Bang_GanGiaTriThatChoO(h, c + 13, item.rTongCap);
            Bang_GanGiaTriThatChoO(h, c + 14, item.rTongThu);
            Bang_GanGiaTriThatChoO(h, c + 15, item.rSoTien);
            Bang_GanGiaTriThatChoO(h, c + 1, item.sTenDonVi_Co);
            Bang_GanGiaTriThatChoO(h, c + 2, item.sTenDonVi_No);
            Bang_GanGiaTriThatChoO(h, c + 3, item.sTenTaiKhoan_Co);
            Bang_GanGiaTriThatChoO(h, c + 4, item.sTenTaiKhoan_No);

            Bang_GanGiaTriThatChoO(h, c + 22, item.iID_MaDonVi_Co);
            Bang_GanGiaTriThatChoO(h, c + 23, item.iID_MaDonVi_No);
            Bang_GanGiaTriThatChoO(h, c + 28, item.iID_MaDonVi_Co);
            Bang_GanGiaTriThatChoO(h, c + 29, item.iID_MaDonVi_No);
        });
    }
  
    if (Bang_arrMaCot[c] == "sKyHieuNgoaiTe") {
        var iID_MaNgoaiTe = Bang_arrGiaTri[h][c + 20];
        jQuery.ajaxSetup({ cache: false });
        var url = urlServerPath + "KTCT_KhoBac_ChungTuChiTiet/get_ChiTietNgoaiTe?iID_MaNgoaiTe=" + iID_MaNgoaiTe;
        $.getJSON(url, function (item) {
            var rTyGia = item.rTyGia;
            Bang_GanGiaTriThatChoO(h, c + 1, rTyGia);
            var rNguyente = Bang_LayGiaTri(h, "rNguyenTe");
            if (rNguyente != null && rTyGia != null) {
                var sotien = parseFloat(rNguyente) * parseFloat(rTyGia);
                //Bang_GanGiaTriThatChoO(h, c + 1, sotien);
                Bang_GanGiaTriThatChoO_colName(h, "rDTRut", sotien);
            }
...
function BangDuLieu_ThemHangMoi(h, hGiaTri) {
    if (Bang_ChiDoc == false) {
        ...
        Bang_GanGiaTriThatChoO(csH, cs, "");
        cs = Bang_arrCSMaCot["sLoaiST"];
        Bang_GanGiaTriThatChoO(csH, cs, "S");
        cs = Bang_arrCSMaCot["sThuChi"];
        Bang_GanGiaTriThatChoO(csH, cs, "1");

        var i;
        for (i = 0; i < Bang_nC; i++) {
            if (Bang_arrMaCot[i] != 'bDongY' && Bang_arrMaCot[i] != 'sLyDo' && Bang_arrMaCot[i] != 'rTongCap' && Bang_arrMaCot[i] != 'rTongThu') {
        ...
        var maxCT = 0;
        var str_maxCT = "";
        var iLoai = parent.jsLoai;
        if (Bang_nH == 1) {
            var url = urlServerPath + "KTCT_KhoBac_ChungTuChiTiet/getMaxSoChungTuChiTiet?iLoai=" + iLoai;
            $.getJSON(url, function (item) {
...
function BangDuLieu_onKeypress_F2(h, c) {
    if (Bang_ChiDoc == false) {
        ...
        var arrTruongTien = "rDTRut".split(',');
        var i, cs;
...
//    if (Bang_arrMaCot[c] == "sLoaiST") {
//        if (Bang_arrGiaTri[h][c].toUpperCase() != "S" || Bang_arrGiaTri[h][c].toUpperCase() != "T") {
//            alert("Loại Séc/Tiền chỉ được nhập 1 trong 2 giá trị 'S' hoặc 'T'");
//            cs = Bang_arrCSMaCot["sLoaiST"];
//            Bang_keys.fnSetFocus(h, cs);
//            return false;
//        }
//    }
//    if (Bang_arrMaCot[c] == "sThuChi") {
//        if (Bang_arrGiaTri[h][c].toUpperCase() != "X" || Bang_arrGiaTri[h][c] != "1" || Bang_arrGiaTri[h][c] != "2" || Bang_arrGiaTri[h][c] != "3") {
//            alert("Loại Thu/Chi chỉ được nhập 1 trong 4 giá trị 'X,1,2,3'");
//            cs = Bang_arrCSMaCot["sThuChi"];
//            Bang_keys.fnSetFocus(h, cs);
//            return false;
//        }
//    }
    ...
    if (Bang_arrMaCot[c] == "rTyGia" || Bang_arrMaCot[c] == "rNguyenTe") {
        var rTyGia = parseFloat(Bang_LayGiaTri(h, "rTyGia"));
        var rNguyente = parseFloat(Bang_LayGiaTri(h, "rNguyenTe"));
        if (rNguyente != 0 && rTyGia != 0) {
            var sotien = rNguyente * rTyGia;
            //Bang_GanGiaTriThatChoO(h, c + 1, sotien);
            Bang_GanGiaTriThatChoO_colName(h, "rDTRut", sotien);
        }
    }
    if (Bang_arrMaCot[c] == "rDTRut") {
        TinhTongChiTiet();
        return false;
    }

    BangDuLieu_CapNhapLaiHangCha(h, c);
    var csCotGhiChu = Bang_arrCSMaCot["sGhiChu"];
    Bang_arrEdit[h][csCotGhiChu] = true;
...
//Frame chứng từ gọi khi có thay đổi giá trị các trường của chứng từ - ham lay ngay tu tren luoi chi ghi so
...
//function LayGiaTriCapNS(h, c) {
//    var iID_MaDonVi = "", sTenDonVi = "";
//    sTenDonVi = Bang_arrGiaTri[h][c];
//    if (sTenDonVi != "");
//    iID_MaDonVi = sTenDonVi.substring(0, 3);
//    var url = jsKTCT_CapNS;
//    url += "?iID_MaDonVi=" + iID_MaDonVi;
//    $.getJSON(url, function (item) {
//        Bang_GanGiaTriThatChoO(h, c + 2, item.iCapNS);
