...
<body>
    <% 
      ...
      SelectOptionList slTrangThai = new SelectOptionList(dtTrangThai, "iID_MaTrangThaiDuyet", "sTen");
        ...
        String UrlReport = "";
        if (PageLoad == "1")
            UrlReport = Url.Action("ViewPDF", "rptKeToan_QuyetToanNam_DonVi", new { NamLamViec = NamLamViec, ThangLamViec = ThangLamViec, iID_MaDonVi = iID_MaDonVi, TrangThai = TrangThai });
        using (Html.BeginForm("EditSubmit", "rptKeToan_QuyetToanNam_DonVi", new { ParentID = ParentID }))
        {
    %>
    <div class="box_tong">
        <div class="title_tong">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                    <td width="47.9%">
                        <span>Báo cáo quyết toán năm đơn vị </span>
                    </td>
            ...
            <div id="rptMain" style="width: 100%; margin: 0px auto; padding: 0px 0px; overflow: visible;">
                <table width="100%" cellpadding="0" cellspacing="0" border="0" class="table_form2">
                    <tr>
                        ...
                        <td width="10%">
                            <%=MyHtmlHelper.DropDownList(ParentID, slThang, ThangLamViec, "ThangLamViec", "", "class=\"input1_2\" style=\"width: 100%\" onchange=\"ChonDV()\"")%>
                        </td>
                        ...
                        <td width="10%">
                            <div>
                                <%=MyHtmlHelper.DropDownList(ParentID, slTrangThai, TrangThai, "TrangThai", "", "class=\"input1_2\" style=\"width: 100%\" onchange=\"ChonDV()\"")%>
                            </div>
                        </td>
                        <td>
                        </td>
    ...
    <script type="text/javascript">
        ...
        <script type="text/javascript">
            ChonDV();
            function ChonDV() {
                var iThangLamViec = document.getElementById("<%=ParentID %>_ThangLamViec").value;
                var iTrangThai = document.getElementById("<%=ParentID %>_TrangThai").value;
                jQuery.ajaxSetup({ cache: false });
                var url = unescape('<%= Url.Action("ds_DotPhanBo?ParentID=#0&NamLamViec=#1&ThangLamViec=#2&TrangThai=#3&iID_MaDonVi=#4", "rptKeToan_QuyetToanNam_DonVi") %>');
                url = unescape(url.replace("#0", "<%= ParentID %>"));
                url = unescape(url.replace("#1", "<%= NamLamViec %>"));
                url = unescape(url.replace("#2", iThangLamViec));
                url = unescape(url.replace("#3", iTrangThai));
    ...
    <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptKeToan_QuyetToanNam_DonVi", new { NamLamViec = NamLamViec, ThangLamViec = ThangLamViec, iID_MaDonVi = iID_MaDonVi, TrangThai = TrangThai }), "Xuất ra Excel")%>
    <%} %>
