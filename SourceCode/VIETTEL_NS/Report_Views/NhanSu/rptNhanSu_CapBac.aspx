...
<body>
    <% 
        ...
        if (PageLoad == "1")
        {
            //edit
            if (bBaoCaoTH)
                URLView = Url.Action("ViewPDF_TongHop", "rptNhanSu_CapBac", new { DoiTuong = DoiTuong, LoaiIn = LoaiIn, Cap = Cap, Nam = Nam });
            else
                URLView = Url.Action("ViewPDF", "rptNhanSu_CapBac", new { DoiTuong = DoiTuong, LoaiIn = LoaiIn, Cap = Cap, Nam = Nam });
        }
        String urlExport = Url.Action("ExportToExcel", "rptNhanSu_CapBac", new { DoiTuong = DoiTuong, LoaiIn = LoaiIn, Cap = Cap, Nam = Nam });
        using (Html.BeginForm("EditSubmit", "rptNhanSu_CapBac", new { ParentID = ParentID, DoiTuong = DoiTuong, LoaiIn = LoaiIn, Cap = Cap, Nam = Nam }))
        {
    %>
