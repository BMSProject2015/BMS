...
<body>
    <%        
        ...
        {
            urlReport = Url.Action("ViewPDF", "rptQuyetToan_ThuongXuyen_34C", new { iID_MaTrangThaiDuyet = iID_MaTrangThaiDuyet, Thang_Quy = Thang_Quy, RutGon = RutGon, KhoGiay = KhoGiay,ToSo=ToSo,MaND=MaND });
        }
        String ToDaXem = "";
        ToDaXem = Convert.ToString(Request.QueryString["ToDaXem"]);
        ToDaXem += ToSo + ",";
        if (ToDaXem == ",") ToDaXem = "";
        using (Html.BeginForm("EditSubmit", "rptQuyetToan_ThuongXuyen_34C", new { ParentID = ParentID,ToDaXem=ToDaXem }))
        {
        %>
        <div class="box_tong">
             <div class="title_tong">
                ...
                <tr>
                    <td>
                        <span>Báo cáo quyết toán thường xuyên theo đơn vị</span>
                    </td>
                </tr>
             </table>
                </div>
                ...
                <div id="Di2">              
                    ...
                    <td width="7%" class="td_form2_td1"><div>Mẫu rút gọn: </div></td>
                    <td width="4%"><div>
                                <%=MyHtmlHelper.CheckBox(ParentID, RutGon, "RutGon", "", "onchange=\"ChoniThang()\"")%>
                                </div></td>
<td width="8%" class="td_form2_td1"><div>Khổ giấy in : </div></td>
                    <td width="10%"><div>
                                <%=MyHtmlHelper.DropDownList(ParentID, slKhoGiay, KhoGiay, "KhoGiay", "", "class=\"input1_2\" style=\"width: 100%\" onchange=\"ChoniThang()\"")%>                      
                                </div></td>
                    <td width="8%" class="td_form2_td1"><div>Chọn tờ: </div></td>
                    <td width="10%"><div id="<%=ParentID %>_divDonVi">
                    <% rptQuyetToan_ThuongXuyen_34CController rpt = new rptQuyetToan_ThuongXuyen_34CController(); %>
...
<td>
  ...
  <script type="text/javascript">
      function ChoniThang() {
          ...
          var url = unescape('<%= Url.Action("DS_To?ParentID=#0&iID_MaTrangThaiDuyet=#1&Thang_Quy=#2&RutGon=#3&MaND=#4&KhoGiay=#5&ToSo=#6", "rptQuyetToan_ThuongXuyen_34C") %>');
    ...
    <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptQuyetToan_ThuongXuyen_34C", new { iID_MaTrangThaiDuyet = iID_MaTrangThaiDuyet, Thang_Quy = Thang_Quy, RutGon = RutGon, KhoGiay = KhoGiay, ToSo = ToSo, MaND = MaND }), "Xuất ra Excel")%>
